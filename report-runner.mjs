...
var project_config_default = {
  ...
  // with no bump at all.
  version: "1.2.122",
  // Release: engine 1.42.2, a question for a focusable control hidden inside another control, and a faster orientation check on stylesheet-heavy pages.
  // Our own accessibility engine (src/engine/) — the product's only engine.
  engine: {
    name: "pour engine",
    version: "1.42.2"
    // Nested controls: a negative-tabindex control under aria-hidden inside an ARIA widget is now a question, not silence; orientation lock asks each media condition once per audit.
  },
  extension: {
...
function referencedName(element, visited) {
  ...
  return targets.map((target2) => {
    const path6 = new Set(visited);
    if (target2 === element) path6.delete(element);
    return computeName(target2, true, hiddenForName(target2), path6);
...
function computeName(element, inLabelledBy, includeHidden, visited) {
  ...
  if (LABELABLE.has(tag) && element.labels?.length) {
    const text = [...element.labels].map((label2) => computeName(label2, inLabelledBy, hiddenForName(label2), visited)).join(" ").trim();
    if (text) return text;
  }
...
var form_label_default = {
  ...
  evaluate(element) {
    ...
    const labelsText = visibleLabels.map((label2) => nativeLabelName(element, label2)).join(" ").trim();
    const ariaLabel = element.getAttribute("aria-label")?.trim();
...
async function sampledVerdict(source2, foreground, required, doc, overlays = [], under = null, extent = null) {
  ...
  if (source2.tagName === "IMG") {
    range = await imageLuminanceRange(source2.currentSrc || source2.src, overlays, grid, under);
  } else {
  ...
  const rule = {
    ...
    async evaluate(element, { ownText }) {
      ...
      if (alternates.length) {
        ...
        const pick = worstCandidate([...basePaints ? [base] : [], ...alternates.map((a) => ({ ...a, color: dim2(a.color) }))], background);
        foreground = pick.color;
        required = pick.required;
        foregroundOrigin = pick.origin;
      }
...
var nested_interactive_default = {
  ...
  evaluate(element, { isRendered }) {
    ...
    const candidates = [...element.querySelectorAll(INTERACTIVE)].filter((el) => !el.matches(":disabled") && !isInert(el) && !(el.tagName === "INPUT" && el.type === "hidden") && isRendered(el) && !(el.closest('[aria-hidden="true"]') && !(el.hasAttribute("tabindex") && el.tabIndex < 0)) && (el.matches(NATIVE) || el.hasAttribute("tabindex")));
  ...
  evaluate(element) {
    ...
    const badRefs = cells.filter((cell) => {
      ...
      return headers && headers.trim().split(/\s+/).filter(Boolean).some((id) => {
        const target2 = element.getRootNode().getElementById?.(id);
        return !target2 || !cells.includes(target2) || !isHeaderCell(target2);
      });
    });
    if (badRefs.length) {
      return {
...
<body>
  ...
  <header class="top">
    <a class="wordmark" href="https://pour.dev" rel="external"><svg viewBox="0 0 32 32" width="22" height="22" aria-hidden="true" focusable="false"><rect x="2" y="2" width="10.5" height="10.5"/><rect x="19.5" y="2" width="10.5" height="10.5"/><rect x="2" y="19.5" width="10.5" height="10.5"/><rect x="19.5" y="19.5" width="10.5" height="10.5"/></svg><span>pour</span></a>
...
${body}
  ...
  <aside class="consent-banner" id="consent-banner" role="region" aria-label="Cookie consent" hidden>
    <p class="consent-copy">
      ...
      Optional analytics help us understand visits and improve the site.
      They only run if you accept. Declining won\u2019t affect your experience.
      Analytics cookies last one year.
    </p>
