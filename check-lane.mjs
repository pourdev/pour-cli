...
function paintToSrgb(cssColor) {
  ...
  try {
    ...
    const [r, g2, b, a] = probe.getImageData(0, 0, 1, 1).data;
    return { r, g: g2, b, a: a / 255 };
  } catch {
    probeUnavailable = true;
    return null;
  }
...
function luminance({ r, g: g2, b }) {
  return 0.2126 * channelLuminance(r) + 0.7152 * channelLuminance(g2) + 0.0722 * channelLuminance(b);
}
...
function glyphNotWords(element, text, accessibleName2) {
  ...
  const name = accessibleName2(host);
...
function pseudoCue(element, which) {
  let p2;
  try {
    p2 = getComputedStyle(element, which);
  } catch {
    return null;
  }
  const content = p2.content;
  if (!content || content === "none" || content === "normal" || p2.display === "none") return null;
  if (p2.visibility !== "visible" || parseFloat(p2.opacity) === 0) return null;
  const matrix = (p2.transform ?? "none").match(/^matrix(3d)?\((.*)\)$/);
  if (matrix) {
    const n = matrix[2].split(",").map(parseFloat);
    const [a, b, c, d] = matrix[1] ? [n[0], n[1], n[4], n[5]] : n;
    if (a === 0 && b === 0 || c === 0 && d === 0) return null;
  }
  const width = parseFloat(p2.width);
  const height = parseFloat(p2.height);
  const glyph = /^["'].+["']$/s.test(content) && parseFloat(p2.fontSize) > 0 || /^(url|counter|counters|attr|image-set)\(/.test(content);
  const paintedBackground = (parseColor(p2.backgroundColor)?.a ?? 0) > 0 || backgroundImagePaints(p2, Number.isNaN(width) ? null : { width, height }) === "yes";
  const paintedBorder = SIDES.some((side) => sidePaints(p2, side));
  if (paintedBackground || paintedBorder) {
    if (Number.isNaN(width) || Number.isNaN(height)) return glyph ? "glyph" : "unclear";
    const extra = (edges) => edges.reduce((sum, edge) => sum + (parseFloat(p2[edge]) || 0), 0);
...
function focusSuppressors(doc) {
  ...
  const covered = (subject, provider) => provider.subjects.some((p2) => p2 === subject || p2 === "*" || p2 === "");
  const settle = () => {
...
function g(num, name) {
  return { guideline: true, num, name };
}
function p(num, name, type, anchor) {
  return { num, name, type, anchor };
}
var provisions = (() => {
  const list = [];
  let guideline = null;
...
var provisionByNum = new Map(provisions.map((entry) => [entry.num, entry]));
var RULE_REQUIREMENTS = {
...
function requirementsForRule(ruleId) {
