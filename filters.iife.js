...
`;function ct(e=document){let r=e.defaultView,t=e.documentElement,c=le(e),f=je(e,c),o=null,n=null,d=null,g=null,h=null,m=null,b=null,A=null,x="none",L="none",$=0,P=0,N=0,E=0,O=null;function S(){if(e.getElementById("pour-filter-styles"))return;let w=e.createElement("style");w.id="pour-filter-styles",w.dataset.pourAudit="filter",w.textContent=lt,e.head.appendChild(w)}function y(){if(e.getElementById("pour-vision-filter-defs"))return;let w="http://www.w3.org/2000/svg",v=e.createElementNS(w,"svg");v.setAttribute("id","pour-vision-filter-defs"),v.setAttribute("width","0"),v.setAttribute("height","0"),v.setAttribute("focusable","false"),v.setAttribute("aria-hidden","true"),v.dataset.pourAudit="filter",v.style.position="absolute",v.style.pointerEvents="none";let R=e.createElementNS(w,"defs");for(let[K,U]of Object.entries(ve)){let Y=e.createElementNS(w,"filter");Y.setAttribute("id",`pour-vision-filter-${K}`),Y.setAttribute("color-interpolation-filters","linearRGB");let j=e.createElementNS(w,"feColorMatrix");j.setAttribute("type","matrix"),j.setAttribute("values",U),Y.appendChild(j),R.appendChild(Y)}v.appendChild(R),e.body.appendChild(v)}let i=w=>{let v=e.createElement("div");return v.className=w,v.dataset.pourAudit="filter",e.body.appendChild(v),v};function a(){N=0,t.style.setProperty("--pour-vision-x",`${$}px`),t.style.setProperty("--pour-vision-y",`${P}px`),t.style.setProperty("--pour-vision-r",`${Math.min(r.innerWidth,r.innerHeight)}px`)}function s(w){$=w.clientX,P=w.clientY,N||(N=r.requestAnimationFrame(a))}function k(w){let v=w.touches[0];v&&s(v)}function p(w){for(let R of xe)t.classList.remove(`pour-vision-filter-${R}`);if(ke.has(x)&&(e.removeEventListener("mousemove",s),e.removeEventListener("touchmove",k)),n?.stop(),n=null,d?.stop(),d=null,g?.stop(),g=null,o?.remove(),o=null,x=ne[w]!==void 0?w:"none",x==="none"){L==="none"&&(t.style.filter="");return}L!=="none"&&ge("none"),S(),y();let v=ne[x]||"none";t.style.filter=v==="none"?"":v,xe.has(x)&&(t.classList.add(`pour-vision-filter-${x}`),o=i("pour-vision-filter-overlay"),o.dataset.filter=x,x==="floaters"&&(n=Ge(e,o)),x==="migraineAura"&&(d=Ke(e,o)),x==="glossyScreen"&&(g=Ue(e,o))),ke.has(x)&&($=r.innerWidth/2,P=r.innerHeight/2,a(),e.addEventListener("mousemove",s),e.addEventListener("touchmove",k,{passive:!0}))}function M(){E=0,t.style.setProperty("--pour-sensory-x",`${$}px`),t.style.setProperty("--pour-sensory-y",`${P}px`),t.style.setProperty("--pour-sensory-r",`${Math.min(r.innerWidth,r.innerHeight)}px`)}function B(w){$=w.clientX,P=w.clientY,E||(E=r.requestAnimationFrame(M))}function W(w){let v=w.touches[0];v&&B(v)}function z(w){if(e.getElementById("pour-sensory-injected-style")?.remove(),!w)return;let v=e.createElement("style");v.id="pour-sensory-injected-style",v.dataset.pourAudit="filter",v.textContent=w,e.head.appendChild(v)}function _(){if(e.querySelector(".pour-sensory-washout-char"))return;let w=e.createTreeWalker(e.body,NodeFilter.SHOW_TEXT,null),v=[];for(;w.nextNode();)v.push(w.currentNode);for(let R of v){let K=R.textContent;if(!K.trim())continue;let U=R.parentElement;if(!U||U.closest("script,style,noscript,[data-pour-audit]"))continue;let Y=e.createDocumentFragment();for(let j of K)if(j===" "||j===`
`||j==="	")Y.appendChild(e.createTextNode(j));else{let Q=e.createElement("span");Q.textContent=j,Q.style.opacity=(.3+Math.random()*.7).toFixed(2),Q.className="pour-sensory-washout-char",Y.appendChild(Q)}U.replaceChild(Y,R)}}function q(){for(let w of e.querySelectorAll(".pour-sensory-washout-char"))w.replaceWith(w.textContent);e.body.normalize()}let G=[[0,0],[0,18],[4.5,13.8],[7.2,19.5],[9.9,18.4],[7.3,12.9],[13,12.9]],ee=new Map,l=0,u=0,C=null,T=0,F=0,D=0,I=0,V=0,Z=0;function pe(w,v,R){let K=`${w}:${v}:${R}`,U=ee.get(K);if(U)return U;let Y=e.createElement("canvas");Y.width=w,Y.height=w;let j=Y.getContext("2d");if(!j)return"auto";let Q=Math.round(w/2);j.save(),j.translate(Q+v,Q+R),j.scale(1.15,1.15),j.beginPath(),j.moveTo(G[0][0],G[0][1]);for(let ae=1;ae<G.length;ae++)j.lineTo(G[ae][0],G[ae][1]);j.closePath(),j.restore(),j.lineWidth=3,j.lineJoin="round",j.strokeStyle="#fff",j.stroke(),j.fillStyle="#000",j.fill();let re=`url("${Y.toDataURL("image/png")}") ${Q} ${Q}, auto`;return ee.set(K,re),re}function ce(w){let v=w.timeStamp||Date.now(),R=v-D;if(D&&R>0){let K=Math.hypot(w.clientX-T,w.clientY-F);I=I*.8+K/R*1e3*.2}T=w.clientX,F=w.clientY,D=v}function he(w,v){if(!V)return V=w+v.minGap+Math.random()*(v.maxGap-v.minGap),[0,0];let R=w-V;if(R<0)return[0,0];if(R>v.dur)return V=w+v.minGap+Math.random()*(v.maxGap-v.minGap),Z=Math.random()*Math.PI*2,[0,0];let K=1-R/v.dur,U=v.size*K*K;return[Math.cos(Z)*U,Math.sin(Z)*U]}function de(w){l=r.requestAnimationFrame(de);let v=C;if(!v)return;let R=(w-u)/1e3,K=0,U=0;if(v.freq&&v.amp){let re=2*Math.PI*v.freq,ae=Math.max(0,1+(v.intent||0)*Math.min(1,I/700)),De=v.amp*ae;K+=(Math.sin(re*R)*.7+Math.sin(re*1.63*R+1.1)*.3)*De,U+=(Math.cos(re*.97*R+.6)*.7+Math.sin(re*2.11*R+2.3)*.3)*De}if(v.spasm){let[re,ae]=he(w,v.spasm);K+=re,U+=ae}let Y=v.bitmap/2-14,j=Math.max(-Y,Math.min(Y,Math.round(K))),Q=Math.max(-Y,Math.min(Y,Math.round(U)));t.style.cursor=pe(v.bitmap,j,Q)}function mt(w){Le(),C=w,u=r.performance?r.performance.now():Date.now(),I=0,D=0,V=0,Z=Math.random()*Math.PI*2,Re(w.hide?`
      html, :not(html) { cursor: none !important; }
      [data-pour-audit], [data-pour-audit] * { cursor: auto !important; }
    `:`
      :not(html) { cursor: inherit !important; }
      [data-pour-audit], [data-pour-audit] * { cursor: auto !important; }
    `),!w.hide&&(e.addEventListener("mousemove",ce,{passive:!0}),l=r.requestAnimationFrame(de))}function Le(){l&&r.cancelAnimationFrame(l),l=0,C=null,V=0,e.removeEventListener("mousemove",ce),Re(null),t.style.cursor=""}function Re(w){if(e.getElementById("pour-sensory-cursor-style")?.remove(),!w)return;let v=e.createElement("style");v.id="pour-sensory-cursor-style",v.dataset.pourAudit="filter",v.textContent=w,e.head.appendChild(v)}function ht(){$e();let w=()=>{O=r.setTimeout(()=>{A&&(A.classList.add("pour-sensory-spike-flash"),r.setTimeout(()=>{A?.classList.remove("pour-sensory-spike-flash"),w()},150))},3e3+Math.random()*8e3)};w()}function $e(){O&&(r.clearTimeout(O),O=null)}function ge(w){let v=se[L];if(v?.overlay&&t.classList.remove(`pour-sensory-filter-${L}`),v?.hostClass&&t.classList.remove(v.hostClass),v?.mouseTracked&&(e.removeEventListener("mousemove",B),e.removeEventListener("touchmove",W)),v?.injectScript&&q(),v?.cursorJitter&&Le(),v?.viewportOrigin&&bt(),v?.loupe&&Ne(),v?.lens&&f[v.lens]?.remove(),h?.stop(),h=null,m?.stop(),m=null,b?.stop(),b=null,$e(),A?.remove(),A=null,z(null),L=se[w]?w:"none",L==="none"){x==="none"&&(t.style.filter="");return}x!=="none"&&p("none"),S();let R=se[L];t.style.filter=R.css&&R.css!=="none"?R.css:"",R.hostClass&&t.classList.add(R.hostClass),(R.overlay||R.mouseTracked||L==="sensorySpike")&&(A=i("pour-sensory-filter-overlay"),A.dataset.filter=L,R.overlay&&t.classList.add(`pour-sensory-filter-${L}`)),R.mouseTracked&&($=r.innerWidth/2,P=r.innerHeight/2,M(),e.addEventListener("mousemove",B),e.addEventListener("touchmove",W,{passive:!0})),R.injectCSS&&z(R.injectCSS),L==="sensorySpike"&&ht(),R.injectScript&&_(),R.cursorJitter&&mt(R.cursorJitter),R.viewportOrigin&&gt(),R.loupe&&yt(R.loupe),R.lens&&f[R.lens]?.apply(),R.forcedColours&&(h=rt(e)),R.fingertip&&(m=ot(e,R.fingertip)),R.magnifier&&(b=st(e,R.magnifier))}let ie=0;function Fe(){ie=0,t.style.setProperty("--pour-motion-origin",`${r.scrollX+r.innerWidth/2}px ${r.scrollY+r.innerHeight/2}px`)}function ue(){ie||(ie=r.requestAnimationFrame(Fe))}function gt(){Fe(),r.addEventListener("scroll",ue,{passive:!0}),r.addEventListener("resize",ue)}function bt(){r.removeEventListener("scroll",ue),r.removeEventListener("resize",ue),ie&&(r.cancelAnimationFrame(ie),ie=0),t.style.removeProperty("--pour-motion-origin")}let be=null;function yt(w){Ne(),be=nt(e,{...w,point:()=>({x:$,y:P})})}function Ne(){be?.stop(),be=null}let vt=()=>({vision:x,sensory:L});function xt(){p("none"),ge("none")}return{applyVision:p,applySensory:ge,clear:xt,state:vt}}var ur=new Set(["text","search","url","tel","email","password","number","date","datetime-local","month","time","week",""]),fr=new Set(["input","select","textarea","button","meter","output","progress"]);function mr(e){return e.replace(/[\uE000-\uF8FF\u{F0000}-\u{FFFFD}\u{100000}-\u{10FFFD}\u200B-\u200D\u2060\uFEFF]/gu,"").trim()?e:""}function dt(e){return mr(me(e,!1,!1,new Set))}function ut(e){for(let t=e;t;t=Se(t))if(t.getAttribute?.("aria-hidden")==="true"||getComputedStyle(t).display==="none")return!0;let r=getComputedStyle(e).visibility;return r==="hidden"||r==="collapse"}function hr(e,r){let t=e.getAttribute?.("aria-labelledby");if(!t)return null;let c=e.getRootNode(),f=t.split(/\s+/).filter(Boolean).map(o=>c.getElementById?.(o)).filter(Boolean);return f.length?f.map(o=>{let n=new Set(r);return o===e&&n.delete(e),me(o,!0,ut(o),n)}).join(" ").replace(/\s+/g," ").trim():null}function me(e,r,t,c){if(c.has(e))return"";if(c.add(e),!r){let d=hr(e,c);if(d)return d}let f=e.getAttribute("aria-label")?.trim();if(f)return f;let o=e.tagName.toLowerCase();if(o==="img"||o==="area"){let d=e.getAttribute("alt")?.trim();if(d)return d}if(fr.has(o)&&e.labels?.length){let d=[...e.labels].map(g=>me(g,r,ut(g),c)).join(" ").trim();if(d)return d}if(o==="input"||o==="select"||o==="textarea"){if(e.type==="submit"||e.type==="reset"||e.type==="button"){let d=(e.value??e.getAttribute("value")??"").trim();if(d)return d}if(e.type==="image"){let d=e.getAttribute("alt")?.trim();if(d)return d}if(r&&(o==="textarea"||ur.has(e.type))){let d=(e.value??"").trim();if(d)return d}if(e.type==="submit")return"Submit";if(e.type==="reset")return"Reset"}let n=gr(e,t,r,c).replace(/\s+/g," ").trim();return n||(e.getAttribute("title")??e.getAttribute("placeholder")??"").trim()}function gr(e,r,t,c){let f=e.shadowRoot?e.shadowRoot.childNodes:e.childNodes;return pt(e,"::before",r)+ft(f,r,t,c)+pt(e,"::after",r)}function pt(e,r,t){if(e.namespaceURI==="http://www.w3.org/2000/svg")return"";let c=getComputedStyle(e,r);if(!t&&(c.display==="none"||c.visibility==="hidden"||c.visibility==="collapse"))return"";let f=c.content;if(!f||f==="none"||f==="normal")return"";let o=f.match(/\/\s*"((?:[^"\\]|\\.)*)"\s*$/);if(o)return o[1].replace(/\\(.)/g,"$1");let n=f.match(/^"((?:[^"\\]|\\.)*)"$/);return n?n[1].replace(/\\(.)/g,"$1"):""}function ft(e,r,t,c){let f="";for(let o of e){if(o.nodeType===3){f+=o.textContent;continue}if(o.nodeType!==1)continue;let n=o.tagName.toLowerCase();if(n==="script"||n==="style"||n==="noscript"||n==="template")continue;if(!r){if(o.getAttribute("aria-hidden")==="true")continue;let g=getComputedStyle(o);if(g.display==="none"||g.visibility==="hidden"||g.visibility==="collapse")continue}if(n==="slot"){let g=o.assignedNodes?.()??[];f+=ft(g.length?g:o.childNodes,r,t,c);continue}if((n==="img"||n==="area")&&o.getAttribute("alt")===""&&!o.getAttribute("aria-label")?.trim()&&!o.getAttribute("aria-labelledby"))continue;let d=me(o,t,r,c);f+=n==="img"||n==="area"||o.hasAttribute("aria-label")||o.hasAttribute("aria-labelledby")?` ${d} `:d}return f}return Tt(br);})();
