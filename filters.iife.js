...
`||m==="	")y.appendChild(e.createTextNode(m));else{let R=e.createElement("span");R.textContent=m,R.style.opacity=(.3+Math.random()*.7).toFixed(2),R.className="pour-sensory-washout-char",y.appendChild(R)}L.replaceChild(y,i)}}function s(){for(let a of e.querySelectorAll(".pour-sensory-washout-char"))a.replaceWith(a.textContent);e.body.normalize()}let g=[[0,0],[0,18],[4.5,13.8],[7.2,19.5],[9.9,18.4],[7.3,12.9],[13,12.9]],x=new Map,T=0,j=0,D=null,C=0,Q=0,z=0,_=0,M=0,H=0;function d(a,r,i){let k=`${a}:${r}:${i}`,L=x.get(k);if(L)return L;let y=e.createElement("canvas");y.width=a,y.height=a;let m=y.getContext("2d");if(!m)return"auto";let R=Math.round(a/2);m.save(),m.translate(R+r,R+i),m.scale(1.15,1.15),m.beginPath(),m.moveTo(g[0][0],g[0][1]);for(let P=1;P<g.length;P++)m.lineTo(g[P][0],g[P][1]);m.closePath(),m.restore(),m.lineWidth=3,m.lineJoin="round",m.strokeStyle="#fff",m.stroke(),m.fillStyle="#000",m.fill();let F=`url("${y.toDataURL("image/png")}") ${R} ${R}, auto`;return x.set(k,F),F}function h(a){let r=a.timeStamp||Date.now(),i=r-z;if(z&&i>0){let k=Math.hypot(a.clientX-C,a.clientY-Q);_=_*.8+k/i*1e3*.2}C=a.clientX,Q=a.clientY,z=r}function b(a,r){if(!M)return M=a+r.minGap+Math.random()*(r.maxGap-r.minGap),[0,0];let i=a-M;if(i<0)return[0,0];if(i>r.dur)return M=a+r.minGap+Math.random()*(r.maxGap-r.minGap),H=Math.random()*Math.PI*2,[0,0];let k=1-i/r.dur,L=r.size*k*k;return[Math.cos(H)*L,Math.sin(H)*L]}function f(a){T=t.requestAnimationFrame(f);let r=D;if(!r)return;let i=(a-j)/1e3,k=0,L=0;if(r.freq&&r.amp){let F=2*Math.PI*r.freq,P=Math.max(0,1+(r.intent||0)*Math.min(1,_/700)),pe=r.amp*P;k+=(Math.sin(F*i)*.7+Math.sin(F*1.63*i+1.1)*.3)*pe,L+=(Math.cos(F*.97*i+.6)*.7+Math.sin(F*2.11*i+2.3)*.3)*pe}if(r.spasm){let[F,P]=b(a,r.spasm);k+=F,L+=P}let y=r.bitmap/2-14,m=Math.max(-y,Math.min(y,Math.round(k))),R=Math.max(-y,Math.min(y,Math.round(L)));l.style.cursor=d(r.bitmap,m,R)}function Y(a){Z(),D=a,j=t.performance?t.performance.now():Date.now(),_=0,z=0,M=0,H=Math.random()*Math.PI*2,se(a.hide?`
    ...
    `:`
      ...
      [data-pour-audit], [data-pour-audit] * { cursor: auto !important; }
    `),!a.hide&&(e.addEventListener("mousemove",h,{passive:!0}),T=t.requestAnimationFrame(f))}function Z(){T&&t.cancelAnimationFrame(T),T=0,D=null,M=0,e.removeEventListener("mousemove",h),se(null),l.style.cursor=""}function se(a){if(e.getElementById("pour-sensory-cursor-style")?.remove(),!a)return;let r=e.createElement("style");r.id="pour-sensory-cursor-style",r.dataset.pourAudit="filter",r.textContent=a,e.head.appendChild(r)}function ve(){le();let a=()=>{B=t.setTimeout(()=>{E&&(E.classList.add("pour-sensory-spike-flash"),t.setTimeout(()=>{E?.classList.remove("pour-sensory-spike-flash"),a()},150))},3e3+Math.random()*8e3)};a()}function le(){B&&(t.clearTimeout(B),B=null)}function re(a){let r=G[S];if(r?.overlay&&l.classList.remove(`pour-sensory-filter-${S}`),r?.hostClass&&l.classList.remove(r.hostClass),r?.mouseTracked&&(e.removeEventListener("mousemove",p),e.removeEventListener("touchmove",c)),r?.injectScript&&s(),r?.cursorJitter&&Z(),r?.focusOrder&&v.focusOrder.remove(),r?.landmarkMap&&v.landmarkMap.remove(),le(),E?.remove(),E=null,o(null),S=G[a]?a:"none",S==="none"){A==="none"&&(l.style.filter="");return}A!=="none"&&U("none"),N();let i=G[S];l.style.filter=i.css&&i.css!=="none"?i.css:"",i.hostClass&&l.classList.add(i.hostClass),(i.overlay||i.mouseTracked||S==="sensorySpike")&&(E=q("pour-sensory-filter-overlay"),E.dataset.filter=S,i.overlay&&l.classList.add(`pour-sensory-filter-${S}`)),i.mouseTracked&&(O=t.innerWidth/2,I=t.innerHeight/2,u(),e.addEventListener("mousemove",p),e.addEventListener("touchmove",c,{passive:!0})),i.injectCSS&&o(i.injectCSS),S==="sensorySpike"&&ve(),i.injectScript&&n(),i.cursorJitter&&Y(i.cursorJitter),i.focusOrder&&v.focusOrder.apply(),i.landmarkMap&&v.landmarkMap.apply()}let he=()=>({vision:A,sensory:S});function ye(){U("none"),re("none")}return{applyVision:U,applySensory:re,clear:ye,state:he}}return Ae($e);})();
